...
export function Slider({
  ...
  useEffect(() => {
    ...
    if (indexRef.current === index) {
      set(() => ({ [axis]: restPos.current, s: 1, config: key => key === axis && { ...trailingSpring, velocity: velocity.current } }))
    } else {
      ...
      set(i => {
        return {
          ...
          s: 1,
          config: key => key === axis && trailingSpring,
  ...
  }, [index, set, root, vertical, axis, height, width, trail, trailingSpring, draggedSpring, trailingDelay])
  ...
  const bind = useDrag(
    ({
      ...
      if (first) {
        ...
        if (Math.abs(mov) > size / 2 || Math.abs(v) > 0.3) {
          indexRef.current = clamp(indexRef.current + (mov > 0 ? -1 : 1), 0, children.length - 1)
        }
        ...
        else set(() => ({ [axis]: restPos.current, s: 1, config: key => key === axis && trailingSpring }))
...
Slider.defaultProps = {
  ...
  trailingSpring: { mass: 1, tension: 280, friction: 60 },
  trailingDelay: 50,
